 ...
 * <p>期望行为：</p>
 * <ul>
 *   <li>每次启动先查询 `demo_user` 表是否已经存在 10 万条数据；</li>
 *   <li>如果已存在（>= 100000），则跳过本次插入；</li>
 *   <li>如果不存在（< 100000），则写入 10 条演示数据。</li>
 * </ul>
 */
@Component
public class DemoDataInitializer implements CommandLineRunner {
    ...
    @Resource
    private DemoUserRepository repo;

    /**
     * 应用启动时检查 demo_user 表数据量。
     *
     * @param args 启动参数（当前逻辑不使用）
     */
    @Override
    public void run(String... args) {
        final long cur = repo.count();
        if (cur >= TARGET) {
            System.out.println("demo_user 已存在 >= 10 万条数据（当前：" + cur + "），跳过插入。");
            return;
        }

        // 不满足目标数据量时，只补充 10万 条固定演示数据。
